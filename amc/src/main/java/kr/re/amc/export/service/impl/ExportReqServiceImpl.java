package kr.re.amc.export.service.impl;
...
@Service("exportReqService")
public class ExportReqServiceImpl extends EgovAbstractServiceImpl implements ExportReqService{
	...
	public List<ExportReqDto> inqDataBoxList(ExportSearchDto exportSearchDto){
		return exportReqDao.inqDataBoxList(exportSearchDto);
	} 
	...
	public ExportReqDto getExportView(Long exportReqSeq) {
		...
		List<ExportApproverInfoDto> exportApproverInfoDto = exportReqDao.inqExportApproverList(exportReqSeq);
		...
		for(ExportApproverInfoDto exportApproverInfoDtos : exportApproverInfoDto) {
			...
			ExportApproverInfoDto exportApproverInfoDto2 = exportReqDao.ExportConfmInfo(exportApproverInfoDtos);
			...
			exportApproverInfoDtos.setTkoutConfmUserId(exportApproverInfoDto2.getTkoutConfmUserId());
			exportApproverInfoDtos.setProcessDt(exportApproverInfoDto2.getProcessDt());
			exportApproverInfoDtos.setTkoutProcessSttusCd(exportApproverInfoDto2.getTkoutProcessSttusCd());
			exportApproverInfoDtos.setTkoutProcessSttusCdNm(exportApproverInfoDto2.getTkoutProcessSttusCdNm());
			
			idx++;
		}
		exportReqDto.setExportApproverInfo(exportApproverInfoDto);
		
		if(!exportReqDto.getTkoutReqstUserId().equals(null)) {
			UserInfoDto userInfoDto = userDao.getUserSeq(exportReqDto.getTkoutReqstUserId());
	...
	public List<ExportReqDto> getUseDataBoxList(ExportSearchDto exportSearchDto){
		...
		List<ExportReqDto> exportReqDto = exportReqDao.inqUseDataBoxList(exportSearchDto);
		

		for(ExportReqDto exportReq : exportReqDto) {
			List<ExportApproverInfoDto> exportApproverInfoDto = exportReqDao.inqBucketList(exportReq.getDtbxSn());
	...
	public ExportReqDto regExport(ExportReqDto exportReqDto) {
		...
		ExportServerInfoDto exportServerInfoDto = new ExportServerInfoDto(exportReqDto);
		
		exportServerInfoDto.setTkoutReqstNo(exportReqDto.getTkoutReqstNo());
		
		exportReqDao.regExportServerInfo(exportServerInfoDto);
		for(ExportApproverInfoDto exportApproverInfoDtos : exportReqDto.getExportApproverInfo() ) {
			...
			exportApproverInfoDto.setDissRspnberUserId(exportApproverInfoDtos.getDissRspnberUserId());
			exportReqDao.regExportApproverInfo(exportApproverInfoDto);
			
			String template =  "mail/exportAccept";
	        String subject = "[서울아산병원] 데이터박스 반출 신청";
	        String[] to = {     userDao.getUserSeq(exportApproverInfoDtos.getDissRspnberUserId()).getEmail()};
	        Context ctx = new Context();
	        ctx.setVariable("userName", exportReqDto.getChargerNm());
	        ctx.setVariable("exportReqSeq", exportReqDto.getTkoutReqstNo());
	        ctx.setVariable("dataBoxSeq", exportReqDto.getDtbxSn());
	        ctx.setVariable("dataBoxName", exportReqDto.getDtbxNm());
	        try {
	...
	@Override
	public ExportReqDto myAdminExportAccept(ExportReqDto exportReqDto) {
	...
	public ExportApproverInfoDto myExportApprove(ExportApproverInfoDto exportApproverInfoDto) {
		...
		if(exportApproverInfoDto.getLastPang().equals("Y")) {
			ExportReqDto exportReqDto = new ExportReqDto();
			
			exportReqDto.setTkoutReqstNo(exportApproverInfoDto.getTkoutReqstNo());
			exportReqDto.setUpdtUserId(AmcUtil.getPrincipal().getUserId());
			
			ExportReqDto exportServerInfoDto = exportReqDao.getExportSvrInfo(exportApproverInfoDto);
			
			//어드민서버 마운트정보, SSH PORT정보
			String filesystem = "/drAnswer/export";
			int PORT = 22;
			...
			String REMOTE_ADDR = exportServerInfoDto.getTkoutSvrIpAdres();
			String USERNAME    = exportServerInfoDto.getTkoutSvrUserId();
			String PASSWORD    = exportServerInfoDto.getTkoutSvrUserPassword();
			try {
				...
				throw new IllegalArgumentException("어드민서버에 접속할 수 없습니다. 접속정보를 확인해 주세요.");
			}
			
			//NAS 복사
			try {
				StringUtil.downloadDir(exportServerInfoDto.getTkoutTrgetCours() , filesystem+"/"+exportServerInfoDto.getTkoutReqstNo()  , channelSftp);
			} catch (Exception e) {
				throw new IllegalArgumentException("반출대상 파일 경로에 파일이 없습니다.");
			}
			
			exportReqDao.ExportApprove(exportReqDto);
			exportReqDao.myExportApprove(exportApproverInfoDto); 
			String template = "mail/exportApprove";
	        String subject = "[서울아산병원] 데이터박스 반출 신청 승인";
	        String[] to = {exportApproverInfoDto.getEmail()};
	        Context ctx = new Context();
	        ...
	        ctx.setVariable("dataBoxName", exportApproverInfoDto.getDtbxNm());
	        try {
				customMailSender.sendMail(template, subject, to, ctx);
			} catch (Exception e) {
			}
	...
	public ExportApproverInfoDto myExportReject(ExportApproverInfoDto exportApproverInfoDto) {
		...
		String template = "mail/exportReject";
        String subject = "[서울아산병원] 데이터박스 반출 신청 거절";
        String[] to = {exportApproverInfoDto.getEmail()};
        Context ctx = new Context();
        ctx.setVariable("userName", exportApproverInfoDto.getChargerNm());
        ctx.setVariable("tkoutReqstNo", exportApproverInfoDto.getTkoutReqstNo());
        ctx.setVariable("dataBoxSeq", exportApproverInfoDto.getDtbxSn());
        ctx.setVariable("dataBoxName", exportApproverInfoDto.getDtbxNm());
        ctx.setVariable("rejectReason", exportApproverInfoDto.getRejectResn());
        try {
			customMailSender.sendMail(template, subject, to, ctx);
		} catch (Exception e) {
	...
	public void myExportDownload(String exportReqSeq, HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		//반출 경로
		String fileSystemSurfix = "/drAnswer/export";
		
		String filesystem = fileSystemSurfix+"/"+exportReqSeq;
		String zipPath = fileSystemSurfix+"/"+exportReqSeq+".zip";
		...
		response.setHeader("Expires", "-1");
		  // 다운로드 위해 추가.
		 FileInputStream fiss = new FileInputStream(zipPath);
		 BufferedInputStream bis = new BufferedInputStream(fiss);
		 ServletOutputStream so = response.getOutputStream();
		 BufferedOutputStream bos = new BufferedOutputStream(so);
		  
		 File fileSizeZip = new File(zipPath);
