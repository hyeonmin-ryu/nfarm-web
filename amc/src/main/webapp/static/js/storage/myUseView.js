...
Vue.component('maincontents', {
    ...
    data: function() {
        return {
            useStrgeId : useStrgeId,
            useReqstSttusSecd:"", // 사용신청 처리상태
            saveInfo: {
            },
            useStorageInfo:{
                openStorageInfo:{},
    ...
    methods: {
        ...
        getMyUseStorageInfo: function () {
            get(TID.SEARCH,
                "/my/management/storage/use/" + this.useStrgeId,
                {},
                this.callback);
        },
        // 목록버튼 클릭
        ...
        cancel: function(){
            post(TID.CANCEL,
                "/my/management/storage/use/cancel/" + this.useStrgeId,
                this.saveInfo,
                this.callback);
        },
        // 거절 이벤트
        onclickReject: function() {
            if(isNull(this.saveInfo.rejectResn)){
                alertMsg("거절 사유는 필수입니다.",this.$refs.rejectResn);
                return;
            }
            //console.log(JSON.stringify(this.saveInfo));
        ...
        reject: function(){
            post(TID.REJECT,
                "/management/storage/use/reject/" + this.useStrgeId,
                this.saveInfo,
                this.callback);
        },
        // 승인 이벤트
        ...
        approve: function(){
            post(TID.APPROVE,
                "/management/storage/use/approve/" + this.useStrgeId,
                this.saveInfo,
                this.callback);
        },
        ...
        searchCallback: function (results) {
            if (results.success) {
                ...
                this.useReqstSttusSecd = results.response.useReqstSttusSecd;
            } else {
                alertMsg(results.error.message);
            }
        },
