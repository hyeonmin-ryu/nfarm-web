...
Vue.component('maincontents', {
    ...
    methods:{
        ...
        // 기존 첨부파일 삭제
        onClickOrgFileDelete: function (fileSn) {
            let idx = this.noticeFiles.findIndex(function(key) {return  key.fileSn === fileSn});
            this.noticeFiles.splice(idx, 1);
            console.log(this.noticeFiles);
        },
        // 취소 클릭(목록 이동)
        onclickCancel: function () {
            if(this.noticeNo) { // 수정중
                location.href = "/board/notice/view?noticeNo="+this.noticeNo;
            } else { // 신규등록
        ...
        onclickSave:function () {
            ...
            if(!this.saveInfo.noticeSj){
                alertMsg("제목은 필수입니다.", this.$refs.title);
                return false;
            }
            if(!this.saveInfo.imprtncYn){
                alertMsg("중요여부는 필수입니다.");
                return false;
            }
            if(!this.saveInfo.noticeCn){
        ...
        saveCallback: function (results) {
            if (results.success) {
                ...
                console.log(document.getElementsByName("uploadFile"));
                ...
                console.log(results.response);
                    this.noticeNo = results.response.noticeNo;
                    frm.append("noticeNo", this.noticeNo);
